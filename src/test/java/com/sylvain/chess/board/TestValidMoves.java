...
public class TestValidMoves {
    ...
    public void testStartingPositions() {
        ...
        final List<Move> allValidMovesForWhite = board.findAllValidMoves(PlayerColor.WHITE);
    ...
    public void testPromotion() {
      ...
      board.printBoard();
      final List<Move> allValidMovesForBlack = board.findAllValidMoves(PlayerColor.BLACK);
  ...
  public void testPreventCheckMovingPiece() {
    ...
    board.addPiece(new Pawn(PlayerColor.WHITE, new Square(2,2)));
    board.addPiece(new Bishop(PlayerColor.WHITE, new Square(5,2)));
  ...
  public void testEnPassant() {
    ...
    final List<Move> blackMoves = board.findAllValidMoves(PlayerColor.BLACK);
  ...
  public void testCastle() {
    ...
    board.addPiece(new Rook(PlayerColor.WHITE, new Square(8, 1)));
    board.printBoard();
    final Set<Move> castles = board.findAllValidMoves(PlayerColor.WHITE).stream().filter(Move::isCastle).collect(Collectors.toSet());
    System.out.println(castles);
    Assert.assertEquals(2, castles.size());
    board.addPiece(new Queen(PlayerColor.BLACK, new Square(8, 4)));
    board.printBoard();
    final Set<Move> castlesWithCheck = board.findAllValidMoves(PlayerColor.WHITE).stream().filter(Move::isCastle).collect(Collectors.toSet());
    System.out.println(castlesWithCheck);
    Assert.assertTrue(castlesWithCheck.isEmpty());
    board.addPiece(new Pawn(PlayerColor.WHITE, new Square(7, 3)));
    board.addPiece(new Pawn(PlayerColor.BLACK, new Square(1, 2)));
    board.printBoard();
    final Set<Move> castlesNotThreatened = board.findAllValidMoves(PlayerColor.WHITE).stream().filter(Move::isCastle).collect(Collectors.toSet());
    System.out.println(castlesNotThreatened);
    Assert.assertEquals(2, castlesNotThreatened.size());
    board.addPiece(new Pawn(PlayerColor.BLACK, new Square(7, 2)));
    board.printBoard();
    final Set<Move> castleOnlyOne = board.findAllValidMoves(PlayerColor.WHITE).stream().filter(Move::isCastle).collect(Collectors.toSet());
    System.out.println(castleOnlyOne);
    Assert.assertEquals(1, castleOnlyOne.size());
    board.addPiece(new Bishop(PlayerColor.WHITE, new Square(2, 1)));
    board.printBoard();
    final Set<Move> castlesNotAnyMore = board.findAllValidMoves(PlayerColor.WHITE).stream().filter(Move::isCastle).collect(Collectors.toSet());
