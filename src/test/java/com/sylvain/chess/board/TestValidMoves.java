...
public class TestValidMoves {
  ...
  public void testCastle() {
    ...
    board.addPiece(new Rook(Color.WHITE, new Square(8, 1)));
    board.printBoard();
    final Set<Move> castles = board.findAllValidMoves(Color.WHITE).stream().filter(Move::isCastle).collect(Collectors.toSet());
    System.out.println(castles);
    Assert.assertEquals(2, castles.size());
    board.addPiece(new Queen(Color.BLACK, new Square(8, 4)));
    board.printBoard();
    final Set<Move> castlesWithCheck = board.findAllValidMoves(Color.WHITE).stream().filter(Move::isCastle).collect(Collectors.toSet());
    ...
    board.printBoard();
    final Set<Move> castlesNotThreatened = board.findAllValidMoves(Color.WHITE).stream().filter(Move::isCastle).collect(Collectors.toSet());
    ...
    board.printBoard();
    final Set<Move> castleOnlyOne = board.findAllValidMoves(Color.WHITE).stream().filter(Move::isCastle).collect(Collectors.toSet());
    ...
    board.printBoard();
    final Set<Move> castlesNotAnyMore = board.findAllValidMoves(Color.WHITE).stream().filter(Move::isCastle).collect(Collectors.toSet());
